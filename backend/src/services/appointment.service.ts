...
const APPT_COLUMNS = `id, dietitian_id, client_id, client_name, status, appt_date::text AS appt_date, time_from, time_to, created_at, updated_at`;
...
export class AppointmentService {
  ...
  async create(dietitianId: string, input: CreateAppointmentInput) {
    const res = await this.db.query(
      `INSERT INTO appointments (dietitian_id, client_id, client_name, status, appt_date, time_from, time_to)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING ${APPT_COLUMNS}`,
      [dietitianId, input.client_id, input.client_name, input.status, input.appt_date, input.time_from, input.time_to]
    );
    return res.rows[0];
  }
