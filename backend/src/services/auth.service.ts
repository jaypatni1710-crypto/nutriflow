...
import { Pool } from 'pg';
import { AuthTokens, JWTPayload, LoginInput, PublicUser, RegisterInput, User } from '../types/auth.types';
import {
  sendApprovalEmail,
...
const REFRESH_TOKEN_EXPIRY_REMEMBER = 30 * 24 * 60 * 60; // 30 days in seconds
...
export class AuthService {
  ...
  private async createRefreshToken(userId: string, rememberMe: boolean): Promise<string> {
    ...
    const expirySeconds = rememberMe ? REFRESH_TOKEN_EXPIRY_REMEMBER : REFRESH_TOKEN_EXPIRY_DEFAULT;
  ...
  async resendVerification(email: string): Promise<void> {
    ...
    if (result.rows.length === 0) return; // Silently succeed to prevent enumeration
  ...
  async forgotPassword(email: string): Promise<void> {
    ...
    if (result.rows.length === 0) return; // Silently succeed
    ...
    await sendPasswordResetEmail(email.toLowerCase(), token, user.first_name);
  }
  ...
  async getPendingAccounts(): Promise<PublicUser[]> {
    const result = await this.db.query(
      `SELECT id, first_name, last_name, email, phone_number, organization_name, account_type,
       ...
       FROM users WHERE account_type = 'dietitian' AND status = 'pending'
  ...
  async approveAccount(userId: string): Promise<void> {
    const result = await this.db.query(
      `UPDATE users SET status = 'active' WHERE id = $1 RETURNING email, first_name`,
      [userId]
    );
    if (result.rows.length === 0) throw new Error('USER_NOT_FOUND');
    await sendApprovalEmail(result.rows[0].email, result.rows[0].first_name);
  }

  async rejectAccount(userId: string): Promise<void> {
    const result = await this.db.query(
      `UPDATE users SET status = 'rejected' WHERE id = $1 RETURNING email, first_name`,
      [userId]
    );
    if (result.rows.length === 0) throw new Error('USER_NOT_FOUND');
    await sendRejectionEmail(result.rows[0].email, result.rows[0].first_name);
  }

  async suspendAccount(userId: string): Promise<void> {
    const result = await this.db.query(
      `UPDATE users SET status = 'suspended' WHERE id = $1 RETURNING email, first_name`,
      [userId]
    );
    if (result.rows.length === 0) throw new Error('USER_NOT_FOUND');
    // Invalidate all refresh tokens for suspended user
    await this.db.query(`DELETE FROM refresh_tokens WHERE user_id = $1`, [userId]);
    await sendSuspensionEmail(result.rows[0].email, result.rows[0].first_name);
  }

  // Users Panel: active / rejected / suspended (no pending users)
  ...
  async changeUserStatus(userId: string, status: 'active' | 'rejected' | 'suspended'): Promise<void> {
    const result = await this.db.query(
      `UPDATE users SET status = $1 WHERE id = $2 RETURNING email, first_name, status`,
      [status, userId]
    );
    if (result.rows.length === 0) throw new Error('USER_NOT_FOUND');
